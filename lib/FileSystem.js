...
class FS {
	...
	addDatabaseFile (name, schema, dbPath) {
		try {
			fs.writeFileSync(path.join(dbPath, name), JSON.stringify({name, schema, data: []}));	
		} catch (error) {
			throw error;
		}
	}
	...
	// FS.removeDoc - delete doc/docs from database

	// requires:
	// modelPath: String = full path to existsing database file
	// doc: Object = full document to delete
	removeDoc (modelPath, doc) {
		const data = this.readDatabaseFile(modelPath).data;

		const newData = data.filter(element => {
			if (util.isDeepStrictEqual(element, doc))
				return null;
