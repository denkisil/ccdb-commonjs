	...
	dataValidByScheme(data) {
		...
		for (const key of Object.keys(this.schema)) {
			...
			if (this.schema[key].unique) {
				const isExist = model.getDocByParams({[key]: data[key]})[0] || null;
				if (isExist != null)
					throw new ExistValueToUniqueField();
			}
		}
...
class SchemaField {
	constructor (fieldParams) {
		...
		this.unique = fieldParams.unique && typeof fieldParams.unique == 'boolean' && !this.defaultValue ? fieldParams.unique : false;
